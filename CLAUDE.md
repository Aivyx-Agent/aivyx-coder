...
| `aivyx-team` | Nonagon-style multi-agent team schema (`TeamConfig`/`TeamMember`), validation, and attenuation logic (`effective_deny_paths`, `tool_allowlist_is_subset`/`effective_tool_allowlist`) for a future multi-agent team capability. Deliberately zero-dependency on any other workspace crate. Phase 1 ("Foundation") only — schema + validation + attenuation logic. Now consumed by `aivyx-core`'s `delegate_to_specialist` module (`compute_specialist_registry` calls `effective_tool_allowlist`), but still not reachable from the actual binary — no `agent_builder.rs` wiring yet |
