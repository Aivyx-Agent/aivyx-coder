...
| `aivyx` (`crates/aivyx`) | The binary — `agent_builder.rs` builds `Agent` + every collaborator identically regardless of frontend (only the `PermissionPrompter` differs); `main.rs` wires config → backend → agent → either the TUI or, behind `--acp`/`--mcp-server`, `aivyx-acp`/`aivyx-mcp-server`; `routing.rs` wraps the backend in a `RoutedBackend` when `[routing] enabled = true` (off ⇒ the very same `Arc`, tested with `Arc::ptr_eq`), rejects cloud endpoint kinds and a `[routing.endpoints.backend]` table, and runs discovery at startup |
