...
manipulated. This project is unrelated to `Rust/aivyx-pa` (the other, much
larger Aivyx PA agent platform) beyond shared naming and authorship — don't
...
| `aivyx-team` | Nonagon-style multi-agent team schema (`TeamConfig`/`TeamMember`), validation, and attenuation logic (`effective_deny_paths`, `tool_allowlist_is_subset`/`effective_tool_allowlist`) for a future multi-agent team capability. Deliberately zero-dependency on any other workspace crate. Phase 1 ("Foundation") only — schema + validation + attenuation logic. Consumed by `aivyx-core`'s `delegate_to_specialist` module (`compute_specialist_registry` calls `effective_tool_allowlist`), and — since the entry-point branch — conditionally registered onto the running agent in `agent_builder.rs` when `[team] enabled = true` (`TeamSettings`, off by default); a custom-roster config format is still separate, later scope |
| `aivyx-config` | `Settings` — XDG config loading, defaults, `0600` writing |
| `aivyx-core` | `Agent`/`AgentConfig`/`AgentEvent`, the turn loop, `EditFormat` (native tool-call JSON vs. prompted SEARCH/REPLACE), `council` (multi-model `/council` mode), `session` (persistence + `--resume`), `edit_blocks` (SEARCH/REPLACE parsing), `routing_commands` (`/models`, `/model`); main-loop calls carry a `RouteHint` (`code_edit` by default, a team member's `task` via `set_route_task`), `/architect` routes as `plan` when no `[architect]` is configured |
...
| `aivyx` (`crates/aivyx`) | The binary — `agent_builder.rs` builds `Agent` + every collaborator identically regardless of frontend (only the `PermissionPrompter` differs); `main.rs` wires config → backend → agent → either the TUI or, behind `--acp`/`--mcp-server`, `aivyx-acp`/`aivyx-mcp-server`; `routing.rs` wraps the backend in a `RoutedBackend` when `[routing] enabled = true` (off ⇒ the very same `Arc`, tested with `Arc::ptr_eq`), rejects cloud endpoint kinds and a `[routing.endpoints.backend]` table, and runs discovery at startup when `discover = true` |
