...
- **Tier tool sets (verified against real code, not the design doc's original partial list):**
  - `plan` = `read_file`, `grep`, `glob`, `git_read`, `find_references`, `go_to_definition`, `memory_read`, `set_tasks`, `web_fetch`, `web_search`, `list_mcp_resources`, `read_mcp_resource`, `list_mcp_prompts`, `get_mcp_prompt`.
  ...
  - Excluded from **every** tier, always: `delegate_task` (recursion), `repl_start`/`repl_send`/`repl_stop` (unreachable after an ephemeral session ends — same reasoning `delegate_task`'s own `sub_agent_registry` already uses), and every dynamically-bridged `mcp__<server>__<tool>` adapter (confused-deputy risk — a remote MCP caller must not transitively reach a third-party MCP server the operator configured for a different purpose).
